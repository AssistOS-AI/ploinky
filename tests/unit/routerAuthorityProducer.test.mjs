...
test('authority helper uses a pinned image, fixed non-root user, and mandatory immutable cleanup', () => {
    ...
    assert.match(source, /const AUTHORITY_HELPER_IDLE_SCRIPT = `setTimeout\(\(\) => process\.exit\(0\), \$\{ROUTER_AUTHORITY_HELPER_MAX_LIFETIME_MS\}\);`;/);
    ...
    assert.match(source, /authority helper identity or confinement could not be proven/);
    assert.match(source, /reconcileAuthorityHelperByName/);
    assert.match(source, /AUTHORITY_HELPER_RECONCILE_ATTEMPTS/);
    assert.match(source, /reapStaleAuthorityHelpers\(commandRunner, runtime, helperImageId\)/);
    assert.match(source, /\[\s*'ps', '-a', '--no-trunc'/);
    assert.match(source, /ROUTER_AUTHORITY_HELPER_MAX_LIFETIME_MS \+ AUTHORITY_HELPER_STALE_GRACE_MS/);
    assert.match(source, /removeProvenAuthorityHelper/);
    assert.match(source, /actualBodyBytes/);
    ...
    assert.ok(confinement > 0 && confinement < start && start < running && running < registration);
