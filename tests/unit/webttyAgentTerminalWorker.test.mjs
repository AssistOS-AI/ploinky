...
const BASH_WRAPPER_COMMAND = 'if [ -f /etc/ploinky/webtty-env.sh ]; then . /etc/ploinky/webtty-env.sh || exit 124; fi; PS1=\'$PWD $ \'; export PS1; /bin/bash --noprofile --norc; ploinky_webtty_status=$?; case "$ploinky_webtty_status" in 126|127) exit 124 ;; *) exit "$ploinky_webtty_status" ;; esac';
const SH_WRAPPER_COMMAND = 'if [ -f /etc/ploinky/webtty-env.sh ]; then . /etc/ploinky/webtty-env.sh || exit 124; fi; PS1=\'$PWD $ \'; export PS1; /bin/sh -i; ploinky_webtty_status=$?; exit "$ploinky_webtty_status"';
