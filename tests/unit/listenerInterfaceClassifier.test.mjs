...
test('managed interface classifier admits only exact owned bridge gateways and loopback', () => {
    const calls = [];
    const run = (args) => {
        calls.push(args);
...
test('classifier clears previously accepted gateways when runtime inspection fails', () => {
    let timestamp = 0;
    let fail = false;
    const run = (args) => {
