...
# `ploinky enable agent` recreates the agent runtime (forceRecreate) and
# re-attests the Router authority, so repeating a completed enable is not a
# harmless no-op. Each enable therefore records its own completion marker and a
# retry resumes at the first step that has not been recorded yet.
enable_agent_once() {
  local marker="TEST_POST_ROUTER_ENABLED_$1"
  shift
  local description="$1"
  shift
  if [[ "${!marker:-0}" == "1" ]]; then
    test_info "Skipping already completed enable: ${description}."
    return 0
  fi
  test_info "${description}"
  ploinky enable agent "$@" || return 1
  write_state_var "$marker" "1"
  printf -v "$marker" '%s' "1"
}
...
enable_fast_suite_agents_after_router() {
  ...
  enable_agent_once "OPENAI" \
    "Enabling OpenAI test agent ${TEST_REPO_NAME}/${TEST_OPENAI_AGENT_NAME}." \
    "${TEST_REPO_NAME}/${TEST_OPENAI_AGENT_NAME}" || return 1

  enable_agent_once "TO_DISABLE" \
    "Enabling agent ${TEST_AGENT_TO_DISABLE_QUALIFIED}." \
    "$TEST_AGENT_TO_DISABLE_QUALIFIED" || return 1

  enable_agent_once "HEALTH" \
    "Enabling agent ${TEST_REPO_NAME}/${TEST_HEALTH_AGENT_NAME}." \
    "${TEST_REPO_NAME}/${TEST_HEALTH_AGENT_NAME}" || return 1

  enable_agent_once "ALIAS" \
    "Enabling alias test agent ${TEST_ENABLE_ALIAS_AGENT_NAME} as ${TEST_ENABLE_ALIAS_AGENT_ALIAS}." \
    "$TEST_ENABLE_ALIAS_AGENT_NAME" as "$TEST_ENABLE_ALIAS_AGENT_ALIAS" || return 1

  enable_agent_once "GLOBAL" \
    "Enabling agent ${TEST_GLOBAL_AGENT_NAME} in global mode." \
    "$TEST_GLOBAL_AGENT_NAME" global || return 1

  enable_agent_once "DEVEL" \
    "Enabling agent ${TEST_DEVEL_AGENT_NAME} in devel mode." \
    "$TEST_DEVEL_AGENT_NAME" devel "$TEST_REPO_NAME" || return 1
