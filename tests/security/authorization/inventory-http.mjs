...
export function customHttpInventory(catalog, repos) {
    ...
    add('OnlyOfficeAgent', 'onlyOffice', 7000, 'GET', '/control/office/session', 'onlyOffice/src/routes/control.mjs', "'/control/office/session'", workspace, 'GET creates an editing session: require disposable document and verify same-role/Confidential ownership.');
    add('OnlyOfficeAgent', 'onlyOffice', 9100, 'GET', '/internal/document/:token', 'onlyOffice/src/routes/storage.mjs', "'/internal/document/'", internal, 'Container-local storage listener; public Router alias must not bypass signed session token.');
    add('OnlyOfficeAgent', 'onlyOffice', 9100, 'POST', '/internal/callback/:token', 'onlyOffice/src/routes/storage.mjs', "'/internal/callback/'", internal, 'Requires test-owned document callback token and signed DocumentServer JWT; never fabricate a success from invalid token.');
    add('OnlyOfficeAgent', 'onlyOffice', 8080, '*', '/*', 'onlyOffice/src/proxy/editor-proxy.mjs', 'export ', publicProtocol, 'DocumentServer binary route namespace remains opaque: public assets, editor transport and JWT document authorization require image-source route inventory.', 'http+websocket-family');
    ...
    for (const [method, suffix, needle] of [['GET', 'sign-in', '`${ROOT}/sign-in`'], ['POST', 'credential', '`${ROOT}/credential`'], ['POST', 'cancel', '`${ROOT}/cancel`'], ['GET', 'confirmation', '`${ROOT}/confirmation`'], ['POST', 'start', '`${ROOT}/start`'], ['GET|POST', 'resume/:handle/:action?', 'const match = url.pathname.match']]) a('userPersistoAgent', 7000, method, `/service/auth/google/${suffix}`, 'service/googleAuth.mjs', needle, publicProtocol, 'Only local protocol rejection and state isolation are in scope; never contact or attack OAuth provider.');
