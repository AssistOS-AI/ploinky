...
export async function bootstrapAgentLibRuntime({
    ...
    // A host process that already carries a validated contract (a nested tool
    // launched by the CLI, for example) inherits it rather than re-selecting.
    if (String(env[AGENTLIB_ENV.dir] || '').trim()) {
        bootstrapped = validateProvidedContract({ env, fsApi, expectedDir: null });
        return bootstrapped;
    }
