 ...
 * material derived/signed for that id (DS014) — never the master key, never the
 ...
 * derived identity (DS012/DS014). Dropped names are logged (never their values).
...
export function stripReservedAgentEnv(env) {
    ...
    if (dropped) {
        console.error(`[ploinky] ignored reserved agent env name(s) ${dropped.join(', ')} from runtime config — these are router-managed (DS012/DS014).`);
    }
    return env;
}
