...
import { ROUTING_FILE, PLOINKY_WORKSPACE_ROOT, PLOINKY_DIR } from '../services/config.js';
...
import { stopAndRemove } from '../services/docker/containerFleet.js';
import { collectLiveAgentContainers, containerExists } from '../services/docker/index.js';
...
function removeMarketplaceAgentContainer(containerName) {
    ...
    try {
        ...
        throw new Error(`Failed to remove container '${name}': ${error?.message || error}`);
    }
...
function removeMarketplaceAgentFromRegistry(agentRef) {
    const ref = normalizeMarketplaceAgentRef(agentRef);
    const resolved = resolveEnabledAgentRecord(ref);
    if (!resolved || !resolved.containerName || !resolved.record) {
        return {
            status: 'not-found',
            requested: ref
        };
    ...
    const map = workspaceSvc.loadAgents();
    const record = map?.[resolved.containerName];
    ...
    delete map[resolved.containerName];
    workspaceSvc.saveAgents(map);
    ...
    if (method === 'POST' && !route.resource) {
        ...
        try {
            ...
            if (action === 'deactivate_agent') {
                const ref = normalizeMarketplaceAgentRef(body?.agentRef);
                const result = removeMarketplaceAgentFromRegistry(ref);
                if (result?.status === 'removed' && result.containerName) {
                    const removed = removeMarketplaceAgentContainer(result.containerName);
                    if (!removed) {
                        sendMarketplaceError(res, 409, 'agent_deactivation_blocked', `Container '${result.containerName}' still exists after removal attempt.`);
                        return true;
                    }
                }
                if (result?.status && result.status !== 'disabled' && result.status !== 'removed' && result.status !== 'static-removed') {
