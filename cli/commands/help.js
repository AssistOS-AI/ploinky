...
▶ LOCAL DEVELOPMENT
  ...
  update [folderPath]            Verified fast-forward update of Ploinky, AgentLib, repos, projects, and skills
...
${lifecycleHelpLines(surface).join('\n')}
  ...
  Agent dependencies: start/enable/restart reuse verified immutable trees; reinstall rebuilds one agent.
...
function showDetailedHelp(topic, subtopic, subsubtopic, { surface = 'core' } = {}) {
    const helpContent = {
        ...
        'var': {
            description: 'Set a workspace variable (stored encrypted in .ploinky/data/.secrets)',
        ...
        'vars': {
            description: 'List workspace variables (from encrypted .ploinky/data/.secrets)',
            syntax: 'vars',
            examples: [ 'vars' ]
        },
        ...
        'reinstall': {
            ...
            subcommands: {
                'agent': {
                    ...
                    description: 'Re-creates one exact enabled agent registration and rebuilds its dependency tree from empty npm state. Other aliases and shared seeds keep their trees. The previous runtime stays admitted if the rebuild or readiness fails.',
