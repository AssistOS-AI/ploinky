...
function showDetailedHelp(topic, subtopic, subsubtopic, { surface = 'core' } = {}) {
    const helpContent = {
        ...
        'start': {
            ...
            notes: 'Reads manifest of static agent: applies repos{} (clone+enable) and enable[] (enable agents). First run needs agent and port. An explicit --profile selects and persists the workspace profile before startup; direct in-box starts that omit it retain the active workspace profile. --branch <branch> applies to the static agent\'s repo, every manifest dependency repo, AND the achillesAgentLib source. achillesAgentLib is selected from <workspace>/achillesAgentLib when that directory exists (never modified by Ploinky) and otherwise from a managed workspace generation of the canonical repository; --branch validates a local checkout against the requested branch and resolves a managed source to one exact remote commit. When that branch is absent, --branch-fallback default keeps repos on their configured ref and selects the locked achillesAgentLib commit; --branch-fallback fail aborts. --repo-branch <repo>=<branch> (repeatable) reconciles an existing named repo before manifest traversal and overrides that repo when traversal installs it; --reset-repos hard-resets targeted managed repos to the refreshed remote branch.'
        },
        'status': {
