...
function showDetailedHelp(topic, subtopic, subsubtopic, { surface = 'core' } = {}) {
    const helpContent = {
        ...
        'update': {
            ...
            notes: '`update` is the same full workflow as `update all`. The current directory is the selected update folder unless folderPath is supplied. A Ploinky checkout is pulled with git pull --rebase --autostash only when it is inside that folder or the command is launched from inside the checkout; an out-of-scope Ploinky checkout is skipped. The workflow still advances the selected achillesAgentLib source, updates .ploinky/repos, and updates git repositories discovered recursively from folderPath. Inside a Ploinky box, the read-only source self-pull is skipped while writable runtime dependencies, managed repos, projects, and skills continue updating. Missing or unreachable remotes in discovered project repositories are logged and skipped instead of failing the full update; bulk updates continue after individual repository or skills failures and repeat their details in the final error summary without aborting the remaining workflow. Explicit `update repo <name>` failures still return an error. `update`, `update all`, `update repos`, and `update repo <name>` refresh `AchillesCopilotBasicSkills`, `DocumentationSkills`, and `PloinkySkills` into eligible installed .ploinky/repos entries, maintaining `.claude` compatibility and the managed `.gitignore` block; each source repo and all skills-only repos are skipped. `update repos` also advances the achillesAgentLib source: a local <workspace>/achillesAgentLib checkout is revalidated and reported but never pulled or reset, while a managed source stages a new immutable generation. Inside a Ploinky box the outer host owns that source, so the in-box update skips it. Discovered workspace folders can define `ploinky-skills-manifest.json`; when present, that file must be an array of objects with url/name/branch/skills and selects the exact skills to install into `.agents/skills` for that workspace folder. Existing skills caches must match the requested source URL; a conflicting cache is reported without replacing it. Explicit manifest branches are selected without fallback, and uncommitted changes prevent a branch switch. In an interactive Ploinky session, a detected Ploinky self-update is deferred: close the session, run `ploinky update`, then restart Ploinky so the new changes are visible.'
        },
        ...
        'start': {
            ...
            notes: 'Reads manifest of static agent: applies repos{} (clone+enable) and enable[] (enable agents). First run needs agent and port. An explicit --profile selects and persists the workspace profile before startup; direct in-box starts that omit it retain the active workspace profile. --branch <branch> applies to every default boot repo (AchillesIDE, AchillesCLI, copilot-agents), the static agent\'s repo and every manifest dependency repo. achillesAgentLib is selected from <workspace>/achillesAgentLib when that directory exists (never modified by Ploinky; --branch only checks that it is on the requested branch) and otherwise from the copy bundled in the Box image, which normally matches the commit pinned in ploinky-box/dependencies.lock.json; when it differs, Ploinky warns on stderr and uses the image\'s bundled commit, and PLOINKY_AGENTLIB_STRICT_PIN=1 makes the difference fatal (any value other than 0 or 1 is rejected whenever Ploinky selects the Box image\'s bundle). When that branch is absent, --branch-fallback default keeps repos on their configured ref and logs it; --branch-fallback fail aborts. --repo-branch <repo>=<branch> (repeatable) reconciles an existing named repo before manifest traversal and overrides that repo when traversal installs it; --reset-repos hard-resets targeted managed repos to the refreshed remote branch.'
        },
        'status': {
