...
function showDetailedHelp(topic, subtopic, subsubtopic, { surface = 'core' } = {}) {
    const helpContent = {
        ...
        'start': {
            ...
            notes: 'Reads manifest of static agent: applies repos{} (clone+enable) and enable[] (enable agents). First run needs agent and port. An explicit --profile selects and persists the workspace profile before startup; direct in-box starts that omit it retain the active workspace profile. --branch <branch> applies to the static agent\'s repo and manifest dependency repos. A managed release takes achillesAgentLib solely from its immutable PLOINKY_RELEASE_DESCRIPTOR; PLOINKY_AGENTLIB_REF is not a managed release override. Other repos use their default branch unless --branch-fallback fail requests an abort. --repo-branch <repo>=<branch> (repeatable) reconciles an existing named repo before manifest traversal and overrides that repo when traversal installs it; --reset-repos hard-resets targeted managed repos to the refreshed remote branch.'
        },
        'status': {
