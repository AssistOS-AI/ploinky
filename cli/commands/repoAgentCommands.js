...
async function enableAgent(agentName, mode, repoNameParam, alias, authMode) {
    if (!agentName) throw new Error('Usage: enable agent <name|repo/name> [isolated|global|devel [repoName]] [--auth none|sso|guest] [as <alias>]');
    const { shortAgentName, repoName, alias: resolvedAlias, auth } = agentsSvc.enableAgent(agentName, mode, repoNameParam, alias, authMode);
