    ...
    const helpContent = {
        ...
        'enable': {
            ...
            subcommands: {
                ...
                'agent': {
                    syntax: 'enable [agent] <name|repo/name> [isolated|global|devel [repoName]] [--auth none|sso|guest] [as <alias>]',
                    ...
                    examples: [
                        ...
                        'enable agent demo --auth guest',
                        'enable agent demo global',
                        'enable agent demo devel simulator',
                        'enable agent demo as demo2'
                    ],
                    notes: 'If --auth is omitted, no auth is applied unless the agent manifest declares a default auth mode. Built-in pwd/local password login was removed; install an SSO provider agent and run `ploinky sso enable <providerAgent>` for browser login. Enable starts the agent and updates its route, but does not replace the primary/static agent configured by start. Aliases must be unique; commands like reinstall/disable should target the alias when multiple containers exist.'
                }
            }
        },
        'expose': {
