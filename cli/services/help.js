...
function showDetailedHelp(topic, subtopic, subsubtopic) {
    const helpContent = {
        ...
        'update': {
            ...
            notes: '`update` is the same full workflow as `update all`: it runs git pull --rebase --autostash for the Ploinky checkout, .ploinky/repos, and git repositories discovered recursively from folderPath. Without folderPath, discovery starts at the current working directory. Missing or unreachable remotes in discovered project repositories are logged and skipped instead of failing the full update; managed .ploinky/repos updates remain strict. `update repos` updates only installed .ploinky/repos and their achillesAgentLib packages. Discovered project repositories also receive refreshed AchillesCopilotBasicSkills copies and managed .gitignore entries, even when their remote update was skipped. In an interactive Ploinky session, a detected Ploinky self-update is deferred: close the session, run `ploinky update`, then restart Ploinky so the new code is loaded.'
        },
