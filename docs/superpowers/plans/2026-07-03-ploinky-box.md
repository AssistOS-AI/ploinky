...
- Isolation contract: **never `--privileged`**, never auto-escalate; box runs `--user podman --device /dev/fuse --security-opt seccomp=unconfined` (+ `--security-opt label=disable` whenever the **engine** reports SELinux enabled — query the engine (`podman info '{{.Host.Security.SELinuxEnabled}}'` / docker `SecurityOptions`), never the client's `getenforce`: on macOS the podman-machine VM enforces SELinux while the Mac has none).
