...
- Isolation contract: **never `--privileged`**, never auto-escalate; box runs `--user podman --device /dev/fuse --device /dev/net/tun --security-opt seccomp=unconfined` (+ `--security-opt label=disable` whenever the **engine** reports SELinux enabled — query the engine (`podman info '{{.Host.Security.SELinuxEnabled}}'` / docker `SecurityOptions`), never the client's `getenforce`: on macOS the podman-machine VM enforces SELinux while the Mac has none).
...
Run: `bash /Users/danielsava/work/file-parser/ploinky/container/wrapper-tests.sh`
Expected: `wrapper-tests: 20 passed, 0 failed`, exit 0.
...
Expected: 10/10 definition tests pass (a tenth `default-local-llm` test landed upstream on 2026-07-03); 20/20 wrapper tests pass; both repos clean except ploinky's pre-existing `M node_modules/achillesAgentLib` line, which must remain unstaged.
