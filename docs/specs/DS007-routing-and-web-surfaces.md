...
| `GET` or `POST /auth/token` | Returns the authenticated browser's token metadata, user, session/origin/host/generation-bound browser-mutation proof, current generation metadata, and local admin-control proof when applicable. Internal service-route authorization remains separate. `POST` may request an SSO refresh and requires CSRF validation. |
