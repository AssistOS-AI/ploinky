...
`container/smoke-http-router.mjs` is a narrow rootless-Podman real-engine routing-probe. It must start one fixture manifest that declares a private `httpServices[].port` target, reach that service only through the selected Router host port, restart it once, and prove before and after restart that the outer boundary remains exactly `127.0.0.1:<selected-router-host-port>:8080/tcp` plus `0.0.0.0:7882:7882/udp`, with neither `8081` nor the service port published.
