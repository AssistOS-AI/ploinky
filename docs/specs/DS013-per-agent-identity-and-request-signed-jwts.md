...
### Question #8: Why does Marketplace startup use synthetic assertion tools?
Response: Marketplace is a router-owned HTTP control surface rather than a target agent MCP tool, but Agent Assertions still need an operation identity that cannot be exchanged across permissions. Separate `marketplace.read` and `marketplace.enable_agent` tool claims bind the token to the narrow lifecycle action while the HTTP request hash binds method, path, query, and exact body. This preserves the existing signed-request model without introducing another credential family.
