...
The model has exactly two authenticated identity types — `user` and `agent` — plus `guest` (a router-issued session without user or agent privileges). A user authenticates to the router through the configured SSO provider agent and holds the base role `user` and, when applicable, `admin`. The legacy base role `local` is mapped to `user` on read by `normalizeRoles` only for compatibility with old token shapes; built-in local password records and `local:<username>` admin identity shortcuts are retired. Admin is resolved from normalized user roles; a guest is never admin. An agent is a Ploinky runtime process that receives a canonical id `agent:<repo>/<agent>` and one secret at startup.
...
| --- | --- | --- | --- | --- |
| Router Session | client → router | router (`session` subkey) | router | `user-session` or `guest-session` / `ploinky-router` |
...
**Router Session** (`cli/server/auth/localService.js`): claims `typ:user-session` or `typ:guest-session`, `iss:ploinky-router`, `aud:ploinky-router`, a stable `sid`, `iat`, `exp`, and `jti`. Guest sessions carry a `gsess_<id>` subject and optional guest scope. Legacy user-session JWT primitives remain for compatibility and safe rejection of old local-auth tokens; built-in local password login no longer mints active user login cookies. Verification (`verifySessionJwt`): signature with the `session` subkey, `aud == ploinky-router`, `typ ∈ {user-session, guest-session}`, `iss == ploinky-router`; then `getSession` checks the persistent revocation list (`sid`/`jti`) and rejects user-session JWTs carrying a legacy local-auth users-variable binding. Raw router session JWTs terminate at the router and are never forwarded to an agent.
