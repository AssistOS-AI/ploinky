...
WebChat must mediate browser input and selected-agent CLI output through the Router-owned runtime. Plain browser input must reach the CLI as newline-terminated text unless the selected agent opts into the versioned `__webchatMessage` envelope. CLI control output must use one complete newline-delimited JSON record with a recognized versioned WebChat marker. The Router must validate accepted session, task, runtime-state, workspace-file, skills, and interaction records before turning them into named SSE events or reconnect snapshots; unaccepted records must not mutate browser state. A pending interaction response must remain bound to the authenticated subscriber, selected runtime, tab, optional page instance, interaction identifier, and declared option or bounded input contract, and must be delivered to the CLI only as a versioned `__webchatInteractionResponse` record.

Slash-command suggestions must remain agent-defined. For the selected enabled agent, WebChat must obtain the command catalog from its Router-mediated MCP endpoint during browser-page initialization, using the MCP initialization sequence, `tools/list`, and the optional `list_achilles_cli_commands` tool. The structured catalog must be cached in the browser and used for local completion rather than attached to every chat message. A new page initialization may obtain a new catalog, and a successful skills change may explicitly refresh it because the agent's available commands may have changed. When the structured tool is absent, WebChat may derive the fallback catalog only from that agent's admitted `execute_*` MCP tools.
