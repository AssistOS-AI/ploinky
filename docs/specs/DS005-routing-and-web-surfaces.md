...
For `/webchat`, the router must require exactly one `workspace-dir`/`workspaceDir` selector naming an existing real non-root directory below the Ploinky workspace, treat `agent` as an explicit agent selector, and preserve ordinary query parameters across browser session endpoints. Before factory lookup or process creation it dispatches structured argv equivalent to `ploinky cli <agent> --workdir <relative-path> -- --dir=/workspace/<relative-path> ...`. Ordinary parameters remain generic single-token provider flags in their original order. The reserved agent/workspace aliases plus `tabId`, `sessionId`, and `pageInstanceId` are never provider arguments or runtime-identity inputs. Missing, duplicate, root, traversal, nonexistent, file, symlink, and outside selectors fail with stable sanitized errors; there is no root fallback, host-path substitution, shell-string launch, or direct `manifest.cli` container fallback. The CLI path remains generic and may auto-enable the requested agent through the standard global flow.
