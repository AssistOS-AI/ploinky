...
The router must provide first-party browser surfaces at `/webchat`, `/dashboard`, and `/status`. Their fallback asset directories live under `cli/server/<surface>/`. `/webchat` relies on the router login flow and the authenticated router session; it does not accept surface-specific token login. `/dashboard` and `/status` are local control surfaces and require a real authenticated local-admin session on an allowed control Host. They do not accept an invitation, component token, Agent Assertion, Router Request, private assertion, LiveKit JWT, or localhost provenance as administrator identity. Every control mutation additionally requires the exact browser Origin and a session-bound CSRF proof. Dashboard mutations use this stricter control-surface guard directly rather than the route-generation mutation guard used by policy-routed browser endpoints; the control Host has no policy route plan to commit. Asset resolution may also consult the static host root and `webLibs/`, but the documented fallback implementation for the first-party surfaces lives under `cli/server/`.
