...
- `POST /whitelist/command` is the single administrative endpoint for the router's access-control policy (DS014). It is authenticated, never whitelistable, and handles `http.whitelist.*` and `mcp.policy.*` namespaces.
- `/<agent>/...` for transparent per-agent proxying after the router-owned paths above have been considered. The router strips the `/<agent>` mount prefix and forwards the remaining path and query string to the route's upstream host port. The target agent owns paths such as `/index.html`, `/agent-card`, `/v1/chat/completions`, `/task`, and any custom HTTP endpoints. `/<agent>/mcp` remains the special MCP proxy path because the router must preserve MCP session mediation and per-agent Router Request minting (DS013) plus MCP tool policy enforcement (DS014) for tool, resource, and task-status operations.

In addition to the surfaces above, the router reserves a set of internal, router-owned paths that are never proxied to an agent and never whitelistable: `/whitelist/command`, `/auth/*`, `/admin/*`, `/metrics`, `/health/internal`, the router-level `/__agent/*`, and any agent-level `/<agent>/__agent/*` control-plane path (reached only by the router via a Router Request; transparent proxying strips caller-supplied identity headers). The router also consults the `httpRoutes` whitelist (DS014) before the agent authentication gate: a `GET`/`HEAD` request whose normalized path matches an enabled, non-internal whitelist entry is served to a guest without authentication; the query string never participates and a `POST` to a whitelisted path is not public. Non-whitelisted protected routes fall through to the normal auth gate with a generic challenge.
