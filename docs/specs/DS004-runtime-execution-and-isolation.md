...
Profiles may declare `server` as an internal HTTP URL for an agent-owned browser service, such as `http://127.0.0.1:3000`. The active profile overlay replaces the default profile's server declaration as one selected upstream. Container runtimes must record the declaration as a container-local server unless the effective network is `network.mode: "host"`; host sandbox runtimes must record it as host-local because they share the host network. This declaration must not imply a `ports` publish and must not replace the default AgentServer route used for MCP, agent-card, and readiness.
