...
Reuse must also be bound to the current workspace runtime record. The recorded runtime, immutable container id, network mode, agent principal, enable generation, and effective instance must match the inspected container. A same-named container with missing or mismatched generation identity must be removed and recreated before startup; this prevents a fresh `.ploinky` state from adopting orphaned containers whose staged bind sources were deleted.
