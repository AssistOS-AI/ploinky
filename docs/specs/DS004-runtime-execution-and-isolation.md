...
Hardware-aware LLM agents opt in through `manifest.llmRuntime.enabled = true`. When opted in, Ploinky resolves the architecture catalog, runs allowlisted accelerator probes with short timeouts (`nvidia-smi -L`, `nvidia-smi --query-gpu=compute_cap --format=csv,noheader`, `nvidia-ctk cdi list`, `/dev/kfd`, `/dev/dri`, `/dev/accel`, `rocminfo`, `amd-smi`, `lspci -nn`, `vulkaninfo --summary`), inspects the container daemon's OCI platform via `docker info`/`docker version`/`podman info --format json`, and selects a compatible architecture record before dependency-cache preparation. Accelerator families require confirmation signals, not just device-file presence: NVIDIA Spark requires a minimum compute capability from `nvidia-smi`, ROCm requires a ROCm tool, Vulkan requires a renderer from `vulkaninfo`, and Intel/OpenVINO requires an Intel device confirmation.

Architecture selection produces a typed runtime policy (platform, memory, cpus, pids-limit, shm-size, ulimit memlock, allowlisted CDI/host devices, allowlisted `securityOpt`, `ipc`, `--gpus`). Ploinky emits those arguments into the `docker run` / `podman run` command and labels the container with `ploinky.llm.architecture`, `ploinky.llm.catalog`, `ploinky.llm.catalogref`, `ploinky.llm.policyhash`, `ploinky.llm.imagedigest`, and `ploinky.reusehash`. Container reuse for LLM agents compares both `ploinky.envhash` and `ploinky.reusehash` against the desired values; the reuse hash includes the runtime startup contract, environment hash, effective network, architecture id, image ref, image digest, OCI platform, runtime policy hash, catalog id, catalog resolved ref, catalog source, redacted catalog repo URL, and catalog requested ref. The single architecture override (`PLOINKY_LLM_ARCHITECTURE_ID`), forced platform (`PLOINKY_LLM_FORCE_PLATFORM`), and forced accelerator family (`PLOINKY_LLM_ACCELERATOR`) are validated against the same typed contract. Raw image override env vars are rejected for LLM runtime selection; catalog image refs remain the source of image authority. Runtime policy validation is never bypassed. There is no per-agent architecture override because architecture selection is host/runtime policy, not agent-owned model policy. Non-LLM agents are untouched.

For LLM runtime agents, Ploinky mounts the agent package at `/workspace`, uses `/workspace` as the container workdir unless the manifest explicitly overrides it, mounts alias-specific model storage at `/models`, and mounts alias-specific runtime state at `/runtime`. Launchers live under `/workspace/modelLaunchers`; long-lived model paths are `/models/hf-cache`, `/models/artifacts`, and `/models/derived`; `/runtime` holds `selected-architecture.json`, launch configs, PIDs, state, and redacted logs. Manifest volumes and runtime resources must not shadow `/workspace`, `/models`, or `/runtime`.

LLM runtime images run one MCP/control service as their image entrypoint: `node /opt/ploinky/runtime-agent/mcp-server.mjs` on container port `9000`. Engines are started on demand by runtime MCP tools and listen on container port `8080`. Ploinky forces both ports to be published on loopback host ports (`127.0.0.1:<host>:9000` and `127.0.0.1:<host>:8080`) and rejects LLM runtime host/container/service network modes that would bypass that publish contract. The runtime sets the clean in-container env names `HF_HOME`, `PLOINKY_MODELS_DIR`, `PLOINKY_DERIVED_DIR`, `PLOINKY_RUNTIME_DIR`, `PLOINKY_LAUNCHERS_DIR`, `PLOINKY_MCP_PORT`, `PLOINKY_INFERENCE_PORT`, `PLOINKY_INVOCATION_AUTH_MODULE=/Agent/lib/invocationAuth.mjs`, and `PLOINKY_REQUEST_HASH_MODULE=/Agent/lib/requestHash.mjs`; retired LLM runtime env names are rejected before container start.
