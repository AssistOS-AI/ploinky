...
### Question #5: Why should default runtime data prefer `.ploinky/`?

Response:
Ploinky already owns `.ploinky/` as the workspace runtime boundary. Keeping default service data, generated config, dependency state, logs, transcripts, and repository clones under that boundary makes `destroy`, cleanup, backup, smoke tests, and repository browsing predictable. Manifests may still declare explicit workspace or host mounts when their runtime contract requires external paths.

## Conclusion

The workspace and repository model centers on a single `.ploinky/` root, repository clones under that root, source metadata for reinstall, and a predictable state layout. The runtime and documentation must continue to describe and preserve that layout as the foundation for all higher-level behavior.
