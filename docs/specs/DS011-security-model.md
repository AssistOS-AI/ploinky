...
HTTP service routes are declared by agent manifests through `httpServices`; the router must not encode product-specific service paths in core handlers. A declaration with `auth: "none"` under `/public-services/...` intentionally bypasses router identity. A declaration with `auth: "guest"` follows the same default policy as a manifest-level guest agent: it honors an existing local login, otherwise mints a scoped guest identity, and forwards a router-issued `__http_service__` invocation token. Guest services that must behave like invite links can set `forceGuest: true` to ignore existing workspace login cookies. Protected service routes under `/services/...` reuse the owning route's auth policy and may pass a compact `x-ploinky-auth-info` header derived from `req.user`. When the downstream service may need to make delegated agent calls, the router includes a router-issued invocation token in that auth-info payload so the service can re-enter `/mcps/<agent>/mcp` with `X-Ploinky-Caller-JWT`. The identity fields in `x-ploinky-auth-info` are not a signed secure-wire grant by themselves; downstream services must trust them only when the request came through the declared router path and must not accept caller-supplied equivalents as authoritative.
