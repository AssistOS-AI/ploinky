...
Agents are isolated from the host by containers, bubblewrap, or macOS Seatbelt. As of the per-agent identity model (DS013), every runtime receives only its own canonical id `PLOINKY_AGENT_ID` before attestation. A credential-capable managed container then receives only its own derived `PLOINKY_AGENT_SECRET`; host/none, bwrap, Seatbelt, and host-hook paths remain principal-only. The shared `PLOINKY_DERIVED_MASTER_KEY` is never injected for invocation signing. Code running inside a credential-capable agent can therefore forge tokens only for that agent (its own secret) — not for another agent — and it still cannot decrypt workspace stores or mint session JWTs. This restores non-repudiation between credential-capable enabled agents within the single-workspace, operator-controlled trust model, while unattested runtimes cannot mint Router-facing credentials at all. The `derived-master` subkey is retained only as the root for agent-OWNED generated secrets (`generatedSecret`/`sharedGeneratedSecret`), not for request authorization. See DS013 for per-agent secret derivation, phased credential injection, the three request-signed JWT families, and the channel-scoped confined relay used to reach host-networked loopback services without restoring a reusable agent secret.
...
`/auth/*` handles login, logout, account, token, and callback flows. `/api/agents/<agent>/users` performs its own local-admin authorization because it must authenticate against the target agent's local-auth policy. `/mcp` is protected by normal route authentication before router-level MCP aggregation. `/<agent>/mcp` defers browser authentication or delegated-caller verification until the JSON-RPC body is available, because secure-wire tokens are body-bound. Browser clients, including WebChat input and slash-command discovery, obtain a mutation proof from `/auth/token?mutationRoute=<routeKey>` and send it in `x-ploinky-browser-csrf-token` on each state-changing agent-first MCP request. On an agent-root or dedicated-service Host, the Router retains the immutable Host-selected route as the authentication authority and admits the requested mutation route only when it belongs to that Host's compiled service closure; `agent=<routeKey>` must not be used to switch the selected public Host identity. The proof is bound to the exact authenticated session, browser origin, immutable generation, selected Host route, and mutation route; clients refresh it once when the router reports a stale generation or invalid proof. Local-session proof derivation uses the signed stable `sid` claim rather than the sliding JWT serialization, retaining revocation identity while allowing authenticated responses to refresh cookie expiry without breaking concurrent browser mutations.
...
The per-agent model (DS013) provides non-repudiation between credential-capable enabled agents within the local trust domain: each such agent holds only its own secret, while principal-only runtimes hold no reusable Router-facing secret. An agent cannot forge a Router Request for another agent, and a permitted agent-to-agent call is authenticated by an Agent Assertion the source signs with its own secret. The security invariant remains that the router is the sole issuer and that agents receive Router Requests only through router-mediated calls. Agent-to-agent access is additionally gated by the fail-closed MCP tool policy (DS014).
...
backend establishes its exact principal and tuple as a mandatory launch step.
Only a managed default/bridge Podman launch that completes generated-local
Router attestation derives and injects the tuple-bound private secret; host/none,
bwrap, Seatbelt, and host-hook paths remain principal-only and cannot make
private authenticated service calls. Identity-store failure cannot degrade a
credential-capable launch to an unbound identity. Semantic health recurs after
startup, and a socket-owner failure inactivates routing before any replacement
attempt.
...
WebChat direct uploads must follow the same workspace confinement rules and narrow them to the resolved WebChat working directory. Directory listing, directory creation, and upload targets must reject absolute caller paths, traversal segments, NUL bytes, Ploinky runtime state (`.ploinky` and `.data`), dependency directories, reserved secret names, symlink components, and any canonical target outside that working directory. Upload quota inventory must use the same target-admissibility filter so runtime, dependency, and reserved-secret files cannot consume or bypass the direct-upload quota. The browser may display only cwd-relative and workspace-relative paths; host absolute paths must never cross the HTTP boundary.
...
Response:
The shared-HMAC model injected one `derived-master` key into every agent, so any agent that read the shared secret could mint tokens for another principal — no non-repudiation between agents. That model is replaced (DS013): every runtime receives only its principal before attestation, and each credential-capable managed container receives only its own `PLOINKY_AGENT_SECRET`, derived from the master via HKDF with the agent id as domain separation. Host/none, bwrap, Seatbelt, and host-hook paths remain principal-only. The router signs each Router Request with the target agent's secret, and permitted agent-to-agent calls are authenticated by an Agent Assertion the source agent signs with its own secret and the router verifies by deriving that agent's secret. One agent reading its own environment can no longer forge tokens for another, and an unattested runtime cannot mint one at all. Confined loopback relay uses a fresh channel-scoped transport key after exact lease and container verification, not the reusable agent secret. This restores non-repudiation between credential-capable enabled agents within the local trust domain; multi-tenant or mutually hostile hosting would still require additional issuer hardening (e.g. asymmetric signatures) but is outside the documented model.
