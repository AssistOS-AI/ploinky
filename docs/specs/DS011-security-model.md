...
Agent-owned generated secrets must derive from `PLOINKY_DERIVED_MASTER_KEY`, not from `PLOINKY_MASTER_KEY` and not from random persistent storage. Manifests declare those secrets with `derive: "derived-master"` env entries, and runtime resources use `{{derivedMasterSecret:NAME}}` when a generated value is needed in a resource template. Ploinky derives each value with a domain-separated label containing the repo name, agent name, and secret name. Cross-agent service credentials that must be identical, such as a shared media-service API key, must opt into a common logical label with `deriveRepoName`, `deriveAgentName`, and `deriveName` rather than relying on the current agent's repo/name. External provider credentials and operator-supplied API keys remain explicitly configured because their values originate outside the workspace.
...
`ensurePersistentSecret()` remains available for legacy generated workspace values that are not agent-owned secrets. Agent-owned generated secrets must use `derive: "derived-master"` or `{{derivedMasterSecret:...}}` instead of generating random persistent values.
