...
When the all-repository `update` flow refreshes workspace folders, each folder that contains `ploinky-skills-manifest.json` must refresh its skills from the manifest sources. The manifest is an array of repository paths or remote URLs; at each `update` run, `.agents/skills/` is reconstructed from those sources, and duplicate skill names are resolved by manifest order so the last listed source wins. A failure to clone/download a manifest skill source, copy the skills, or update `.gitignore` is a command failure, not just an informational warning.
