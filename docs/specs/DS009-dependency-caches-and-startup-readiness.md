...
Readiness answers whether a newly started runtime may receive traffic. Liveness answers whether an already activated runtime should remain available. A fixed wrapper starts the mounted runtime-relay broker for every Node-capable managed container, starts the health broker when the manifest declares script-backed health, and then starts the image/manifest command unchanged. Ploinky submits health requests and cancellation through the per-container control bind, and the Router reaches container loopback through the inode-verified socket in that bind; probing, AgentLib admission, and normal routed traffic create no OCI exec session in the target. Probe scripts must live in the agent root, use a safe filename, return exit code `0` for success, obey their hard deadline, and leave no unproved child process behind. A readiness failure prevents route activation; a liveness failure removes the stale route before recovery.
...
A dependency marked `noWait` may continue in a detached worker only after the main transaction has resolved its identity, graph position, direct dependencies, and final preparation generation. Each worker must use a run-specific id, status file, log file, wave index, immutable registry binding, and bounded deadline. It may wait only for its declared direct dependency barriers and must never infer readiness from an unrelated worker. Once such a barrier releases a consumer, that worker must retain its workspace and network lifecycle leases from process creation through readiness and route activation so a peer route publication cannot transiently remove the active Router generation during the consumer's bootstrap. Publishing a newly ready target from its target-less staged route must use additive preparation and commit under the same apply lock: the exact predecessor remains active while the candidate locator and runtime-only registry fields are captured, and the selector flips only after the complete candidate is durable. The ordinary inactive-generation apply path is forbidden for this transition because a runtime may still be completing its own bootstrap work after a transport-level readiness probe succeeds.
