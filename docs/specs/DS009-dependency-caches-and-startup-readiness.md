...
Readiness answers whether a newly started runtime may receive traffic. Liveness answers whether an already activated runtime should remain available. For a container with script-backed health, a fixed wrapper starts the mounted probe broker in the container's main process tree and then starts the image/manifest command unchanged. Ploinky submits requests and cancellation through the per-container control bind; neither probing nor AgentLib admission creates an OCI exec session in the target. Probe scripts must live in the agent root, use a safe filename, return exit code `0` for success, obey their hard deadline, and leave no unproved child process behind. A readiness failure prevents route activation; a liveness failure removes the stale route before recovery.
