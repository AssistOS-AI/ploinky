...
The optional manifest `llmRuntime` block opts an agent into hardware-aware LLM startup. When `llmRuntime.enabled` is `true`, Ploinky loads an external architecture catalog (default `local-llm-architectures/`, overridable through `PLOINKY_LLM_ARCHITECTURES_PATH` or `PLOINKY_LLM_ARCHITECTURES_REPO`/`_REF`), inspects host hardware and the container daemon's OCI platform, selects a compatible architecture, and substitutes the catalog-selected image for the manifest `container`/`image` field. Catalog image references are literal refs from the catalog and raw image override env vars are rejected rather than bypassing catalog policy. The registry record's `containerImage` must be the effective image actually used to start the container. For LLM runtime agents the registry may also store `manifestImage` when a manifest image resolved before substitution, plus an `llmRuntime` metadata block with architecture id, catalog id/ref, platform, image digest, policy hash, and reuse hash.
