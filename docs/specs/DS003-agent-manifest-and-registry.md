...
Explicit enablement starts the selected runtime before publishing its route. The post-start check must use the backend recorded by the runtime manager: container status for Docker or Podman, and the tracked process PID for Bubblewrap or Seatbelt. A host-sandbox process must never be rejected merely because no same-named OCI container exists. If the selected backend exits during startup, enablement must fail before route state is written.

For the default `isolated` run mode, the registry `projectPath` must resolve to `.data/<agent-or-alias>/` under the workspace root. Aliased instances use the alias as the data directory name so multiple enabled instances do not share a home. Container runtimes and the Linux Bubblewrap backend mount `.data/<agent-or-alias>/` at `/root` and set `HOME=/root` for every enabled agent, regardless of run mode. Isolated agents also receive `WORKSPACE_PATH=/root`. `global` agents keep `projectPath` and `WORKSPACE_PATH` at the workspace root, and `devel` agents keep `projectPath` and `WORKSPACE_PATH` at the selected `.ploinky/repos/<repo>/` checkout.
...
Bulk disable behavior is part of the registry contract. `disable agents-all` must iterate all enabled agent entries in `.ploinky/agents.json` and remove their selected runtime instances using the same backend dispatch as single-agent disable. Registry entries must be removed before teardown so the watchdog cannot restart an intentionally disabled agent. Because removal erases the persisted runtime discriminator, the teardown call must receive a captured record snapshot and use it to distinguish Docker/Podman containers from Bubblewrap/Seatbelt processes. The command reports a final summary of removed, not-found, ambiguous, unchanged, and failed outcomes, and agent homes remain preserved.

The manifest surface may define startup commands, CLI commands, readiness hints, dependencies, profiles, runtime resources, local password defaults, SSO-provider markers, repository bootstrap directives, and enable directives. It may also define OpenAI chat-completions and models-list handlers under `endpoints`, plus open-ended capability metadata used by agent-card and fallback-model surfaces. `endpoints.models` is command-backed like `endpoints.chatCompletions`; it returns a standardized model descriptor list for the agent-as-provider contract. When `endpoints.models` is absent, AgentServer returns a single fallback `default` model so existing agents remain addressable. That fallback must use normalized `manifest.capabilities.tags` when declared and must otherwise use only `generic-agent`; it must not infer narrower tags such as `chat` or `agentic`. Capability metadata is intentionally open-ended JSON: common fields such as tags, summary, usage guidance, and input conventions are useful but not mandatory. RoutingServer exposes agent-card metadata through `/agent-card/<agent>` and also aggregates all successful agent responses at `/agent-card` without validating the returned field shape. Manifest `enable` entries may pull in additional agents and may attach aliases. Manifest `repos` entries may install repositories before dependent agents are resolved. Startup must apply the static agent manifest's repository and enable directives, then recursively apply enabled child manifests' repository and enable directives with cycle protection before building the workspace dependency graph. This lets each manifest own the repositories required by the dependencies it introduces instead of forcing the root manifest to duplicate all transitive repository sources. A `repos` value may be a URL string or an object `{ "url": "...", "branch": "..." }`; the object form lets a manifest pin a dependency repo to a specific branch. When `ploinky start` supplies a branch policy, manifest branch values participate in the resolution order (after explicit `--repo-branch` but before the global `--branch` candidate).
...
Continuing a selected-CLI-owned task is also an explicit activation of the
task's stored provider. If that provider has no ready route, the selected CLI
uses Ploinky's agent-authenticated Marketplace enable action in global mode and
waits for declared readiness before resuming the task. This activation must
remain bound to the provider stored in the task; it must not select a substitute
agent.
...
### Question #13: Why is startup policy separate from `start` and `enable`?
...
### Question #14: Why does the fallback model use `generic-agent` instead of inferred capability tags?

Response:
An agent without a custom model catalog has not declared a narrower model function. `generic-agent` identifies an agent-backed route without claiming chat, tool-use, reasoning, or other behavior that the fallback catalog cannot prove. Agents that need functional routing can declare `manifest.capabilities.tags` or implement `endpoints.models`.
