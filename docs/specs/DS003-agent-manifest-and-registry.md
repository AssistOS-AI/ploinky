...
`ploinky start --profile <name>` is the explicit profile selector across the public box boundary. The public boxed entrypoint selects and forwards `default` when the flag is omitted; a direct in-box start with no explicit selector may continue using the profile persisted in `.ploinky/profile`. Box publish planning applies the selected workspace profile to each enabled graph node and falls back to that manifest's `default` profile when the workspace profile is absent. A `profile` explicitly attached to an `enable` edge is dependency-local and must exist on that child manifest; it must fail with the child reference and available profiles rather than falling back. For boxed publish planning, `explorer`, `AchillesIDE/explorer`, and `AssistOSExplorer/explorer` are equivalent root references.
...
The profile-scoped `openPorts` array is both the agent's inner exposure contract and the allowlist of sockets eligible to cross the Ploinky box boundary by default. For boxed public Explorer starts, the outer wrapper reads the enabled graph and converts each effective-profile entry into an outer publish whose container target is the stable box-side port, not the private agent-container port. Accepted generated claims use TCP or UDP, equal-length port ranges, and a nonzero box-side port, for example `127.0.0.1:8081:8081`, `127.0.0.1:3478:3478/udp`, or `127.0.0.1:7882-7892:7882-7892/udp`.

The graph planner rejects overlapping box-side intervals for the same protocol. This includes wildcard/specific-bind overlaps and different private targets; only a semantically exact duplicate from the same effective graph node is deduplicated. Conflict diagnostics must identify both owners, including their profiles, aliases, bind classes, original declarations, and box-to-private mappings. TCP and UDP claims for the same numeric port remain independent.
