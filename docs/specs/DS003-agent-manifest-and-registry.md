...
The optional boolean `routerAccess.workspaceLogs` grants the exact enabled agent instance access to the Router-private `/api/edge/workspace-logs` operation. The capability is generation-bound and does not publish an HTTP route. The operation can list, read, and search Ploinky-owned Router and Policy logs and can perform their UTC rotation and retention cleanup; it exposes no lifecycle mutation or arbitrary filesystem path.
