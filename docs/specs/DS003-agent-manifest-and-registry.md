...
The optional manifest `llmRuntime` block opts an agent into hardware-aware LLM startup. When `llmRuntime.enabled` is `true`, Ploinky loads an external architecture catalog from `PLOINKY_LLM_ARCHITECTURES_PATH`, from `PLOINKY_LLM_ARCHITECTURES_REPO` plus optional `_REF`, or from the built-in default remote `https://github.com/AssistOS-AI/local-llm-architectures.git` at ref `main`. There is no implicit sibling-checkout fallback. Ploinky then inspects host hardware and the container daemon's OCI platform, selects a compatible architecture, and substitutes the catalog-selected image for the manifest `container`/`image` field. Catalog image references are literal refs from the catalog and raw image override env vars are rejected rather than bypassing catalog policy. The registry record's `containerImage` must be the effective image actually used to start the container. For LLM runtime agents the registry may also store `manifestImage` when a manifest image resolved before substitution, plus an `llmRuntime` metadata block with architecture id, catalog id/ref/source, catalog requested ref, platform, image digest, policy hash, and reuse hash.

LLM runtime packages use profile-scoped `openPorts`, normally `profiles.default.openPorts: ["127.0.0.1:0:9000", "127.0.0.1:0:8080"]`, so the runtime publishes the MCP/control service and engine port on loopback host ports. Legacy `ports` profile fields remain invalid and must fail with a rename error. LLM runtime manifests must not provide startup wrapper fields (`start`, `agent`/`commands.run`, or `entrypoint`); the selected image entrypoint starts `/opt/ploinky/runtime-agent/mcp-server.mjs` on container port `9000`, and engines start on demand on container port `8080`. The agent package is mounted at `/workspace` and used as the default container workdir, and production runtime auth is wired with `PLOINKY_INVOCATION_AUTH_MODULE=/Agent/lib/invocationAuth.mjs` and `PLOINKY_REQUEST_HASH_MODULE=/Agent/lib/requestHash.mjs`.

Catalog selection writes `.data/<agent-or-alias>/runtime/selected-architecture.json` (mounted at `/runtime/selected-architecture.json` inside the container), creates `.data/<agent-or-alias>/models` (mounted at `/models`), and gives the runtime image fixed paths for `/workspace`, `/workspace/modelLaunchers`, `/models/hf-cache`, `/models/artifacts`, `/models/derived`, and `/runtime`. Agent-owned `agent-models.json`, launcher scripts, and capability metadata stay inside the agent's own repository; Ploinky core never parses them. See DS012 for the catalog contract and DS004 for the runtime policy emission rules.
