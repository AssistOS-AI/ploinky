...
When an operator enables an agent, Ploinky must persist a registry record in `.ploinky/agents.json` with enough information to restart that agent reproducibly. That record must retain the repository name, short agent name, run mode, project path, alias when present, and normalized auth policy when SSO or guest auth is in use.
...
The optional `routerAccess.requiredCapability` manifest field declares a single capability string (for example `"explorer.access"`) that every authenticated user session must carry before the router proxies any request to that agent's routes. Capabilities are resolved by the workspace SSO provider agent at login and session refresh and ride on the router session user (`user.capabilities`); the router evaluates the requirement after authentication succeeds and denies with `403 CAPABILITY_REQUIRED` when the capability is absent. Roles never satisfy a capability requirement — an `admin` role with an empty capability list is denied. Like `routerAccess.httpRoutes`, the field is evaluated at runtime from the enabled manifest and is never persisted into policy state. Agents without the field are unaffected. Guest sessions and public/guest route access remain governed solely by the HTTP route access policy (DS014); the capability gate constrains authenticated user sessions only.
...
### Question #13: Why is `routerAccess.requiredCapability` enforced by the router instead of per-request agent authorization calls?

Response:
Decision 2026-07-02: capabilities are resolved once by the SSO provider agent (UserPersisto-class providers) during `sso_handle_callback` and `sso_refresh_session` and carried on the router session user. Enforcing the manifest-declared requirement at the router means a user lacking the capability (for example a `selfRegistered` account without `explorer.access`) is blocked before any agent traffic, without making the provider agent a per-request dependency of every route. Staleness is bounded by the session refresh cycle. The field is a single capability string in this version; agents still own fine-grained domain authorization on top of the gate (DS014). Roles deliberately do not bypass the check so that role and capability models cannot drift apart silently.
