...
Continuing a WebChat task is also an explicit activation of the task's stored
provider. If that provider has no ready route, Ploinky must enable it in global
mode through the same internal lifecycle function used by normal enablement,
publish its route, and wait for its declared readiness protocol before resuming
the task. This activation must remain bound to the provider stored in the task;
