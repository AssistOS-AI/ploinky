...
| Router private listener `8081` | Current-generation agent-to-service HTTP, SSE, and WebSocket calls, plus non-secret Router metadata operations. | It is never an outer publication. Effective authenticated policy and an exact caller instance/enable-generation ACL plus request-bound replay-protected private assertion are all required. Metadata operations such as the bodyless `GET /api/edge/runtime-origins` admit any exact current agent with that assertion, answer only from the captured active lease after revalidating it immediately before replying, and are absent from the public and managed listeners. |
...
An admitted agent-port route may need to reach a service bound to the target container's loopback, including a host-networked service. Every Node-capable managed container starts a fixed `/Agent/server/RuntimeHttpRelay.mjs` broker in its main process tree. The broker accepts framed channels only through `runtime-relay.sock` in that container's exact private `0700` control bind. After an initial current authorization-generation lease commit and exact immutable container, owner, generation, label, network-mode, bind, socket inode, ownership, and mode inspection, the Router connects directly to that socket. Linux exposes the socket as `0600`; the supported macOS container-filesystem projection exposes the same socket inode as `0755` to the Box. Those are the only outer modes admitted. No routed request creates an OCI exec session in the target. A fresh random 32-byte key exists only for that relay channel: it is carried in the framed `HELLO`, never injected into the runtime environment or argv, never echoed in `READY` or logs, and cleared on channel close.
