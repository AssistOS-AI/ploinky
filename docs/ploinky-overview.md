...
- `ploinky enable agent <name|repo/name> [global|devel [repo]] [--auth none|sso|guest] [as <alias>]`: register an agent in `.ploinky/agents.json`. Isolated agents use `.data/<agent-or-alias>/` as their host-side home and work directory.
...
- Built-in local password auth is removed; user accounts and password policy belong to the configured SSO provider agent.
