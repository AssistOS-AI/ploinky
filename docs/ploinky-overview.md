...
- `ploinky update [folderPath]` and `ploinky update all [folderPath]`: select the current directory, or `folderPath` when supplied, as the update folder. Ploinky itself is pulled only when its canonical checkout is inside that folder or the command is launched from inside the checkout. The workflow continues when Ploinky is out of scope: it refreshes `ploinky/node_modules/achillesAgentLib`, updates `.ploinky/repos/`, repairs non-git installed repo directories by recloning on the recorded branch when a source URL is known, refreshes `AchillesCopilotBasicSkills` in eligible installed `.ploinky/repos/` entries, recursively updates discovered git repositories, and refreshes Achilles default skills in those project repositories. Inside a Ploinky box, `/opt/ploinky` is mounted read-only, so the self-pull is skipped while the writable runtime Achilles dependency, managed repositories, workspace repositories, and skills continue to update. Discovered project repositories with missing or unreachable remotes are logged and skipped for the pull step while still receiving default skills.
