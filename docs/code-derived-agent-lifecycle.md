...
Probe script names must be plain filenames in the agent root. Slashes and `..` are rejected. Persistent managed agent containers run with an OCI init reaper. Each script runs behind an in-container hard deadline in a fresh process session; Ploinky identifies the exact execution with a random token and private marker, terminates the whole session after timeout, waits for init to reap it, and fails closed if cleanup cannot be proved. A separate outer client deadline invokes the same exact cleanup path if the Podman client hangs.
...
discovery, aggregate MCP, status, WebChat, broker, and private-service
