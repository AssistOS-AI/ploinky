...
4. Preserve the rootless, unprivileged runtime. Enforce the expected immutable image identity, read-only source mounts, and an init/reaper. Do not use privileged mode, SUID/setuid, file capabilities, or relaxed confinement to make a deployment pass.
   - Operator GPU grant: `ploinky gpu grant nvidia --agent REPO/AGENT` lets nested-agent admission accept one `llmRuntime.runtimePolicy.devices` request, the single CDI device `ploinky.local/gpu=all`, only for the agents the grant names and only while the grant is active. From the grant, such an agent's container gets only `--device ploinky.local/gpu=all`. The grant is recorded on the host, outside the workspace, in `~/.ploinky-box/gpu-grants`.
   - To the Box, an active grant adds only explicit `--device` flags for the discovered NVIDIA device nodes, read-only binds of the driver libraries, `nvidia-smi`, a hookless CDI spec and a grant marker, and one label. It adds no privileges, capabilities, security options, publications or network changes, and never widens invariant 3.
   - The grant is not the only device path in a Box. Independently of it, agents that declare `containerSecurity.nestedPodman` get `--cap-add SYS_ADMIN`, `--cap-add NET_ADMIN`, `--device /dev/fuse`, `--device /dev/net/tun`, `--security-opt label=disable` and the nested-Podman seccomp profile.
