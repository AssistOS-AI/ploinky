...
4. Preserve the rootless, unprivileged runtime. Enforce the expected immutable image identity, read-only source mounts, and an init/reaper. Do not use privileged mode, SUID/setuid, file capabilities, or relaxed confinement to make a deployment pass.
   - Operator GPU grant: `ploinky gpu grant nvidia --agent REPO/AGENT` lets nested-agent admission accept a `llmRuntime.runtimePolicy.devices` request for the single CDI device `ploinky.local/gpu=all`, only for agents the grant names and only while the grant is active. The grant is recorded outside the workspace in `~/.ploinky-box/gpu-grants`. To the Box it adds the discovered NVIDIA device nodes as explicit `--device` flags, read-only binds of the driver libraries and `nvidia-smi` under `/usr/local/nvidia`, a read-only hookless CDI spec, a read-only grant marker, and a label.
   - The grant adds no privileges or capabilities, no publications and no network changes, and it never widens invariant 3. It is not the only device path in a Box: agents that declare `containerSecurity.nestedPodman` already get `/dev/fuse`, `/dev/net/tun`, `SYS_ADMIN` and `NET_ADMIN` inside the Box, independently of any grant.
   - `ploinky gpu revoke` removes the grant. It refuses up front when the workspace records show that an enabled agent requesting the GPU would lose it; otherwise that agent fails admission when the graph restarts, and the previous Box, graph and grant are restored. A changed driver replaces the Box under the workspace lock, and when discovery fails the Box runs without GPU devices and the grant is marked stale.
