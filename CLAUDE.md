...
4. Preserve the rootless, unprivileged runtime. Enforce the expected immutable image identity, read-only source mounts, and an init/reaper. Do not use privileged mode, SUID/setuid, file capabilities, or relaxed confinement to make a deployment pass.
   ...
   - `ploinky gpu revoke` removes the grant; it refuses while an enabled agent that requests the GPU would lose it. A changed driver replaces the Box under the workspace lock, and when discovery fails the Box runs without GPU devices and the grant is marked stale.
